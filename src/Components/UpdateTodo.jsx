// src/components/UpdateTodo.js

import React, { useState } from 'react';
import { useDispatch } from 'react-redux';
import { updateTodo } from '../features/Todo/todoSlice';

function UpdateTodo({ id, initialText, onClose }) {
    const [text, setText] = useState(initialText || '');
    const dispatch = useDispatch();

    const handleUpdate = (e) => {
        e.preventDefault();
        if (text.trim()) {
            dispatch(updateTodo({ id, text }));
            setText('');
            onClose();  // Close the update form after submission
        } else {
            alert('Please enter a valid todo text');
        }
    };

    return (
        <div>
            <h2>Update Todo</h2>
            <form onSubmit={handleUpdate}>
                <input
                    type="text"
                    value={text}
                    onChange={(e) => setText(e.target.value)}
                    placeholder="Update todo text"
                    className="border p-2 rounded"
                />
              <button type="submit" className="ml-2 bg-blue-500 text-white px-5 py-2 rounded  items-center">
    <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 448 512"
        width="20"  // Adjust size as needed
        height="22"  // Adjust size as needed
        fill="currentColor"  // Makes the icon inherit the button's text color
        className="mr-2"  // Adds space between the icon and the "Save" text
    >
        <path d="M64 32C28.7 32 0 60.7 0 96L0 416c0 35.3 28.7 64 64 64l320 0c35.3 0 64-28.7 64-64l0-242.7c0-17-6.7-33.3-18.7-45.3L352 50.7C340 38.7 323.7 32 306.7 32L64 32zm0 96c0-17.7 14.3-32 32-32l192 0c17.7 0 32 14.3 32 32l0 64c0 17.7-14.3 32-32 32L96 224c-17.7 0-32-14.3-32-32l0-64zM224 288a64 64 0 1 1 0 128 64 64 0 1 1 0-128z"/>
    </svg>
    
</button>

            </form>
        </div>
    );
}

export default UpdateTodo;
